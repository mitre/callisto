...
D. Install a task module by placing it's .jar file in the appropriate
   ...
   directory. The personal "tasks" directory is in
   "$(HOME)/.callisto/tasks" (see below). Tasks in the personal tasks
   directory will supersede those in the installation tasks directory
   if they have the same identifier (Each task has a hard coded ID
   which is independant of the name of the .jar file).
   ...
   Solaris, Linux, MacOSX: These system simply use the $HOME variable
                           in your envoronment. Mac users cannot see
...
9. Available and Default Action.
...
they called MAIA.  In Greek mythology, Maia is one of the daughters of
Atlas.  
...
fate, and so transformed her into the constellation Great Bear (Ursa
Major), thus allowing her to gain immortality.  He gave their child,
