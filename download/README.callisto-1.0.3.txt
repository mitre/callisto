...
The initial development of the tool by the MITRE Corporation was
funded by the U. S. government.  The primary development team
consisted of Chad McHenry, Robyn Kozierok, and Laurel Riek, with some
contributions from David Day.  Design and interface guidance was
provided by Lisa Ferro, Janet Hitzeman, Marcia Lazo, Marc Vilain and
David Day.  The project leader for this effort was David Day.
...
II. Using Callisto
   ...
   C. Known Bugs and Limitations
       1. File Locations.
       2. Accessing NIST/Atlas Web site.
       3. Saving annotations to APF.
...
Callisto is distributed as a single compressed archive containg all
the neccissary Java archive files (names ending in '.jar'). Additional
...
files which Callisto will automatically load. The current version of
Callisto recognizes a tasks if it's .jar file is in your
personal "tasks" folder (<HOME>/.callisto/tasks).

Callisto requires network access to read configuration files from the
NIST website. This is a bug we are working to resolve. For computers
behind a firewall, Callisto provides a way to set a proxy to get
...
B. Uncompress the main archive to some location on your machine. The
   ...
   Run Callisto once to automatically set up your personal preferences
   directory, which will make it easier to install task modules.
...
D. Setting Proxy
   ...
   3. Select the 'Advanced' group from the possible preference groups
   4. Enable the the proxy with the checkbox, and enter the name of
      the proxy host, and port number for your proxy server. Contact
      your network administrator for this information.
        eg.  Proxy Host:  proxy.example.com
             Proxy Port:  80

E. Install a task module by placing it's .jar file in your personal
   ".callisto/tasks" directory. They will be automatically loaded by
   Callisto when it starts up.

   1. Windows 95, 98, ME

      These operating systems consider the "C:\" directory your <HOME>
      directory by default. Copy or move task .jar files to
      "C:\.callisto\task".

   2. Windows 2000, XP

      On these operating systems, <HOME> is by default 
      "C:\Documents and Settings\<USERNAME>". Copy or move task .jar
      files to "<HOME>\.callisto\tasks".

   3. Sun Solaris, Linux, MacOSX:

      Copy or move task .jar files to "$HOME/.callisto/tasks". MacOSX
      users cannot see this directory using the Finder, so must do
...
A. Callisto Features
  ...
  evolve.  While every effort will be made to keep documentaion up to
...
tokenization.  To accomodate, Callisto allows text to be swiped in
...
The 'text palette' is a floating window that shows the actions
available to the main text pane as buttons.  The 'default action' is
activated by clicking the middle mouse button in the main text pane.
Buttons in the text palette duplicate the text pane's context pop-up
menu (see above), with a small round toggle button next to each,
indicating which is currently the default.  Actions will become
disabled depending on what the annotator is doing at any given time
(see Selected vs. Swiped above), and if the default action becomes
disabled, clicking the middle mouse button will have no effect.
...
Floating winows are automatically reattched by closing them.  Floating
...
MenuBar.  Of note is the 'Proxy' server which must be set if you are
behind a firewall.  This is due to the current requirement of the
jATLAS code to obtain the latest version of the ATLAS and METADATA XML
DTD's from the NIST web site.  No other network activity is performed.

14. Annotation Highlights and Task Preferences.

Each task defines it's own preferences and annotation hightlight
...
Currently Callisto recognizes and handles four distinct types of
files: Raw text files, arbitrary sgml (including html, xml) files, Ace
Pilot Format (APF) files, and Atlas Interchange Format (AIF) files.
In this version of the tool we have adopted a Windows-like model for
file type identification: the various file extensions (and those
alone) indicate to the tool what kind of data are contained in that
file.  This means that for Callisto to properly handle a given file
type, you will need to make sure that the filenames have extensions
that conform to the following naming conventions:

  AIF (Atlas Interchange Format): *.aif, *.aif.xml, *.aif.*.xml
  APF (ACE Pilot Format):         *.apf, *.apf.xml
  SGML (including HTML and XML):  *.sgml, *.sgm, *.html, *.htm,
                                  *.xml
  Text: All other file extensions.

All data annotated by this version of Callisto are saved in AIF
format.  Loading an APF file will convert the APF ACE annotations
automatically into AIF format.  An APF export facility will be ready
soon.  Note that when loading an APF file, the "signal" file (or
...
the box) of the "Text Palette" option underneath the "Edit" pulldown
menu.
...
word boundaries.  If this default behaviour interferes with sub-word
selections, the user can change this preference from the "Selection
Mode" option underneath the "Edit" pulldown menu (changing it to
...
appriate action is selected from the pop-up menu, (b) the desired
...
as in the mentions table --- these values are all "setfills" that are
set by clicking on the cell and selecting from the pop-up menu.   As an
accellerator, the user can click once on any such setfill cell and the
most-likely default value will be inserted, without requiring one to
actually select it from the pop-up menu.
...
There are a number of known bugs and/or restrictions in this version
of Callisto that can be avoided.

1. File Locations.

The jATLAS system that was developed by NIST is the basis of the
representation and "backing store" of the annotation repository.  This
system makes the strong assumption that signal files (the data to be
annotatated) are canonical signals, and will not be changed in any
way, including their location.  This means that if you move a file
(<filename>) which has been previously annotated by Callisto
(<filename>.aif.xml), or if you move the annotation file, you will not
be able to open it from within Callisto.  Even if the signal text file
*and* the annotation file are moved to the same new directory. This
assumption will eventually be removed from Callisto, but it has not
yet been removed from the current version. The only workaround at this
time is to hand edit the annotation file to point to the new signal
location.

2. Accessing NIST/Atlas Web site.

The jATLAS system also makes the strong assumption that annotation
will take place on a computer that is connected to the internet.  This
assumption will be removed from Callisto, but it has not yet been
removed from this version.  If you are annotating on a machine that
accesses the Internet via a proxy, it is necessary to go to the
Advanced tab in the Preferences widget and set the appropriate values
for your http proxy service.
...
The current version of Callisto requires that the file name reflect
the content of the file. All files must be saved with the ".aif"
extension.  Files with ".sgml" extension will have the sgml stripped
and a corresponding ".txt" file will be created, which will then be
opened for annotation; the original ".sgml" file is not
modified. Files with a ".apf" extension will handled in a manner
similar to ".sgml" files. Future versions of Callisto will determine
what to do with a file by looking at it's content, or by asking the
user.
...
will eventually be set up with an external presence that will be the
focal point for online documentation, mailing lists, bulletin boards,
downloads and bug reports (possibly at SourceForge,
<http://www.sourceforge.net> or OpenChannel,
http://www.openchannelfoundation.org/.)

In the interim, Callisto comments and general questions should be
directed to:
...
more information on ATLAS).  Of course, Atlas is also one of the gods
of ancient Greek mythology.  Subsequently NIST developed the jATLAS
implementation of the ATLAS formalism, and defined a new type
definition language for ATLAS, which they called MAIA.  In Greek
mythology, Maia is one of the daughters of Atlas.  In keeping with
this theme we have named our text annotation tool after another Greek
mythological figure, Callisto.  Callisto consorted with Zeus (after he
tricked her by disguising himself as Artemis).  Their tryst resulted
in a child, Arcas 1, who came to rule Arcadia later in life.  Callisto
was punished for being Zeus' lover, and was turned into a bear by
Hera.  Zeus had pity on Callisto falling to this fate, and so
transformed her into the constellation Great Bear (Ursa Major), thus
allowing her to gain immortality.  He gave their child, Arcas 1, to
Hermes' mother, Maia, one of the Pleiades, who raised him to
adulthood.
