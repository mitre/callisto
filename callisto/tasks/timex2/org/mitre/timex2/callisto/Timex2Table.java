...
public class Timex2Table extends JPanel implements JawbComponent {
  ...
  private static final int PHRASE_COL = 0;
  private static final int VAL_COL = 1;
  private static final int MOD_COL = 2;
  private static final int ANCHOR_DIR_COL = 3;
  private static final int ANCHOR_VAL_COL = 4;
  private static final int SET_COL = 5;
  private static final int NON_SPECIFIC_COL = 6;
  private static final int NO_VAL_COL = 7;
  private static final int NUM_COLS = 8;
  
  private static final String COMPONENT_ID = "simpleTypeEditor";

  private static final String[] ATTRIBS =
    new String[] {"TextExtent", "val", "mod", "anchor-dir", "anchor-val", 
		  "set", "non-specific", "no-val"};

  private static final String[] HEADINGS =
    new String[] {"Phrase", "Value", "Mod", "Anchor Dir", "Anchor Val", "Set",
		  "Non Specific", "No Value"};

  TIMEX2ToolKit toolkit;
  ...
  public Timex2Table (TaskToolKit toolkit) {
    this.toolkit = (TIMEX2ToolKit) toolkit;
    Task task = toolkit.getTask();
    init (task, task.getAnnotationType (TIMEX2Task.TIMEX2_NAME));
  ...
  private void init (Task task, AnnotationType type) {
    ...
    for (int i=1; i<=ANCHOR_VAL_COL; i++) {
  ...
  private class DocPropertyListener implements PropertyChangeListener {
    public void propertyChange (PropertyChangeEvent evt) {
      ...
      if (JawbDocument.FONT_FAMILY_PROPERTY_KEY.equals (name)) {
        argEditor.setTimex2Font (new Font((String) evt.getNewValue(),
