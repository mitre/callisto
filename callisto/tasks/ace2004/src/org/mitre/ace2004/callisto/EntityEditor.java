 ...
 */
public class EntityEditor extends JPanel implements JawbComponent {
  ...
  private static final String COMPONENT_ID = "entityEditor";

  // TODO: order of these attribs & headings is relied upon in EntityModel
  ...
  private static final String[] HEADINGS =
    new String[] {"ID", "Primary Reference", "Mentions",
                  "Type", "Subtype","Class"};
  ...
  /**
   * Creates a new <code>EntityEditor</code> instance.
   */
  public EntityEditor (TaskToolKit toolkit) {
    this.toolkit = (ACE2004ToolKit) toolkit;
    Task task = toolkit.getTask();
    init (task, task.getAnnotationType (ACE2004Task.ENTITY_TYPE_NAME));
  ...
  private void init (Task task, AnnotationType type) {
    ...
    AnnotationTableModel atm =
      new EntityModel (task, type, ATTRIBS, HEADINGS);
    ...
    table.setSelectedAnnotationExchanger(ACE2004Utils.RELATION_MENTION_TYPE,
                                         toolkit.relation2Entities);
    table.setSelectedAnnotationExchanger(ACE2004Utils.RELATION_TYPE,
                                         toolkit.unmentionedRelation2Entities);
   ...
   */
  private class EntityModel extends AnnotationTableModel {
    public EntityModel (Task task, AnnotationType type,
    ...
    public Object getValueAt (int row, int col) {
      if (col == PRIMARY_MENTION_COL) { // primary mention column
        ...
        if (DEBUG > 2) {
          System.err.println ("EE.EntityModel.getValAt: val class="+
                              annot.getClass()+" val="+annot);
          Class c = annot.getClass();
          System.err.println ("   superclass="+c.getSuperclass()+
                              " package="+c.getPackage ()+
                              " declaring="+c.getDeclaringClass()+
                              " inst of annotref?="+
                              (annot instanceof AnnotationRef));
        }
        ...
        return ((AWBAnnotation)annot).getAttributeValue("head.TextExtent");
        
      } else if (col == MENTION_COUNT_COL) {
        AWBAnnotation annot = (AWBAnnotation)getAnnotation (row);
        int size =
          annot.getRegion().getSubordinateSet(ACE2004Utils.ENTITY_MENTION_TYPE).size ();
