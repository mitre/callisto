...
public final class ExportAPF5_1_5 implements Exporter {

  private static final String APF_VERSION = "5.1.5";

  private static final int DEBUG = 0;

  // To support handling external_resource and external_link elements in APF
...
// Exporter
  ...
  private void addEntity(Element doc, AWBDocument awbDoc,
	...
	for (int resCount = 0; resCount < externalResourceCount; resCount++) {
	    ...
	    // not yet encountered.
	    // System.out.println("resCount = " + resCount + " externalResourceCount = " + externalResourceCount);
	    // System.out.println("externalResourceName = " + externalResourceName[resCount]);
	    // System.out.println("externalLinkName = " + externalLinkName);
	    if (externalResourceName[resCount].equals(externalLinkName)) {
		newNameP = false;
	    }
	}
	if (newNameP) {
	    System.out.println("NEW externalResourceName = " + forceString(externalLinkName, "NO_NAME"));
	    System.out.println("NEW externalResourceUrn  = " + forceString(externalLinkUrn, "NO_URN"));
	    externalResourceUrn[externalResourceCount]  = forceString(externalLinkUrn,  "NO_URN");
	    externalResourceName[externalResourceCount] = forceString(externalLinkName, "NO_NAME");
	    externalResourceCount++;
	    System.out.println("new externalResourceCount = " + externalResourceCount);
	}
    }
    ...
    if (primary == null)
      ...
      addMention(entity, awbDoc, primary, true);
    ...
    private String forceString (String inString, String defaultString) {
	if (inString == null) {
  ...
  private void addMention(Element entity, AWBDocument awbDoc,
    ...
    addMention (entity, awbDoc, aceMention, false);
  }
  ...
  private void addMention(Element entity, AWBDocument awbDoc,
    ...
    String primary = Boolean.toString(isPrimary);
  ...
  private void addTimex2(Element doc, AWBDocument awbDoc,
    ...
    String cal          = toString(aceTimex2.getAttributeValue("cal"));
  ...
  private void addRelation(Element doc, AWBDocument awbDoc,
    ...
    if (arg1 == null) {
      ...
      for (int i = 0; i < aceMentions.length; i++) {
        ...
        if (mentionArg1.getAnnotationType().equals(ACE2004Utils.ENTITY_TYPE)) {
          arg1 = mentionArg1;
        } else {
    ...
    if (arg2 == null) {
      ...
      for (int i = 0; i < aceMentions.length; i++) {
        ...
        if (mentionArg2.getAnnotationType().equals(ACE2004Utils.ENTITY_TYPE)) {
          arg2 = mentionArg2;
        } else {
	...
	// HasSubordinates aceMention = (HasSubordinates) aceMentions[i];
      ...
      // RK 3/14/07 if null, must get the entity arg from the parent
      if (mentionArg1 == null) {
        mentionArg1 = arg1;
      }
  ...
  private void addEvent(Element doc, AWBDocument awbDoc,
    ...
    *********************/
    ...
    // Loop over mentions: create standalone, adding structure, but promoting
    // its arguments to the event before adding the mentions to the relation
    for (int i = 0; i < aceMentions.length; i++) {
	...
	if (aceMention != null) {
		...
		.addAttribute("REFID", toString(aceMention.getAttributeValue("ace_id")));
            ...
            if (type.equals(ACE2004Task.ENTITY_TYPE_NAME)) {
              // there is no text extent, skip it (RK 3/14/07)
            } else {
   ...
   * parent level.
   * 
   * RK 3/14/07 Now need to also look at the umentioned_entity_values
   */ 
  private void addArgumentList(Element parent, Element mention,
      ...
      if (DEBUG > 0) {
	  System.err.println("Export.APF5_1_5 addArgumentList parent mention "
			     + parent + " " + mention);
      }
    ...
    while (iter.hasNext()) {
      ...
      if (DEBUG > 0) {
	  System.err.println("Export.APF5_1_5 next = " + next);
      }
	...
	next = ((AnnotationRef) next).getElement();
      ...
      if (DEBUG > 0) {
	  System.err.println("Export.APF5_1_5 aceArgument = " + aceArgument);
      }

      String role = toString(aceArgument.getAttributeValue("role"));
      if (DEBUG > 0) {
	  System.err.println("Export.APF5_1_5 role = " + role);
      }
      ...
      if (value == null) {
        value = (AWBAnnotation) aceArgument.getAttributeValue("unmentioned-entity-value");
        if (value == null) {
      ...
      String type;
      if (DEBUG > 0) {
	  System.err.println("Export.APF5_1_5 value = " + value);
      }
      if (value == null || role == null) {
      ...
      if (type.equals(ACE2004Task.TIMEX2_TYPE_NAME) || 
          type.equals(ACE2004Task.ENTITY_TYPE_NAME)) {
	  // TIMEX2 mentions and entities are the same
          // and Entities are already "promoted" (RK 3/14/07)
	  values.add(value);
      } else {
